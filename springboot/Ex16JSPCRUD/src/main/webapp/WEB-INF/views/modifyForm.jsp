...
<head>
  ...
  <script>
    function checkValue(){
        if(!document.userInfo.userId.value){ //empty, null, 길이0 체크
            alert("이름을 입력하세요.");
            document.getElementById('userId').focus();
            return false; //form태그의 submit통신이 막힘.
        }
        if(!document.userInfo.userPw.value){ //empty, null, 길이0 체크
            alert("암호를 입력하세요.");
            document.getElementById('userPw').focus();
            return false; //form태그의 submit통신이 막힘.
        }
        if(!document.userInfo.userRole.value){ //empty, null, 길이0 체크
            alert("권한을 입력하세요.");
...
<form action="modifyAction" method="post" name="userInfo" onsubmit="return checkValue();">
  ...
  <table border="1">
    ...
    <tr>
      <td>비번</td>
      <td>
        <input value="${member.userPw}" type="password" name="userPw" id="userPw" maxlength="20" />
      </td>
    </tr>
    <tr>
      <td>이름</td>
    ...
    <tr>
      ...
      <td>
        ...
        <select name="userRole">
          <option value="ROLE_USER" <c:if test="${member.userRole eq 'ROLE_USER'}">selected</c:if>
          >일반회원</option>
          <option value="ROLE_ADMIN" <c:if test="${member.userRole eq 'ROLE_ADMIN'}">selected</c:if>>관리자</option>
        </select>
      </td>
    </tr>
    <tr>
      <td colspan="2" align="center">
        ...
        <button type="submit" formaction="/list">회원목록</button>
      </td>
    </tr>
  </table>
</form>
